...
async def yoloTraining (model: None | int = None, size: int | list[int] = 1280, args: list[str] = []):
    ...
    model.train(
        data=os.path.abspath("coco-seg/data.yaml"),
        cfg=os.path.abspath("training/yolo/best_hyperparameters.yaml"),
        patience=100,
        epochs=1000,
        batch=-1,
        imgsz=size,
    ...
    model.export(format="tfjs", imgsz=size, keras=True, int8=True)  # creates '/yolov8n_web_model'